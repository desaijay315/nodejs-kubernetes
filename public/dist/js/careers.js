...
$(document).ready(function () {
    ...
    $('.testimonial').each(function () {
      if ($(this).hasClass('bigFrame')) {
        $(this).find('figure').css({
          'width': ($(this).width() + 18) * 3
        });
      } else {
        $(this).find('figure').css({
          'width': ($(this).width() + 15) * 3
        });
      }
    });
    var defaultIndex = 2;
  ...
  $('.disruptors_text_slider').slick({
    ...
    draggable: false //asNavFor: '.disruptors_slider',
  ...
  $('#applied_post').change(function () {
    var applied_post = $(this).val();
    var location = $('#applied_city option:selected').val();
    careersFilter(applied_post, location);
  });
  $('#applied_city').change(function () {
    var applied_post = $('#applied_post option:selected').val();
  ...
  function careersFilter(cat, loc) {
    try {
      ...
      $('#careersfilter .careersDetails').each(function () {
        var locationFilter = $(this).data('location');
  ...
  $('.job_card .btn-wrap .btn:last-child').click(function () {
    var post_applied = $(this).closest('div.job_card').find('h4').text(); //console.log(post_applied);
